# -----------------------------------------------------------------------------
# Control Plane EC2 Instance - Kubernetes Lab
# lab-vanilla-kubernetes/aws/00-k8s/03-control-plane/terragrunt.hcl
#
# Deploys the Kubernetes control plane EC2 instance.
...
inputs = {
  ...
  user_data = <<-EOF
    ...
    # Set hostname
    hostnamectl set-hostname k8s-control-plane-01

    # Update /etc/hosts
    echo "127.0.1.1 k8s-control-plane-01" >> /etc/hosts
  EOF
